...
- **Logging-but-continuing on a degenerate `n_class <= 1` case:** superseded as of this phase's second verification pass (see Open Questions Q1, RESOLVED) — a logged duplicate is still a degenerate synthetic sample and does not satisfy SMOTE-03's literal wording. The adopted fix is to skip synthesis entirely for that class (zero synthetic rows), not to produce an explicitly-labeled duplicate.
...
**How to avoid:** `SMOTE-03` explicitly requires both cases to log an explicit warning and avoid crash/degenerate output. As of this phase's Open Question 1 (RESOLVED, see below), the `n_class <= 1` case now SKIPS synthesis entirely rather than falling through to an interpolation attempt — this means the empty-borderline-pool fallback (`class_idx[c]`) is only ever reached when `n_class >= 2`, so it can no longer independently reproduce the duplicate-degeneration bug: interpolating between two or more genuinely distinct same-class points is ordinary, non-degenerate SMOTE behavior.
...
| A4 | ~~For the pre-existing `n_class <= 1` case (SMOTE-03's second clause), the recommended fix is to log a warning and continue producing an explicit, labeled duplicate (not to skip synthesis entirely and leave the class under-sampled)~~ **SUPERSEDED** — see Open Questions Q1, RESOLVED below. After the second verification pass, the user explicitly decided option (b) (skip synthesis entirely) instead, because a logged-but-continued duplicate does not literally satisfy SMOTE-03's "no degenerate synthetic samples" wording. | Pitfall 3 | Medium (historical) — "no crash and no degenerate synthetic samples" in `ROADMAP.md`'s exact wording was read two ways; the ambiguity is now resolved by explicit user decision (skip), not by planner default. Training on an under-balanced class relies on the existing class-weight mechanism (`cw_binary`/`cw_expert` in `mode_train()`) to compensate — this reliance is now the accepted, intended behavior for this edge case, not a fallback of last resort. |
...
1. **What exactly does "no degenerate synthetic samples" mean for the pre-existing `n_class <= 1` case (SMOTE-03)?**
   ...
   - Recommendation (superseded — see RESOLVED below): Default to (a) — log and continue with an explicitly-labeled duplicate — since it is the smaller, more localized change (a warning line, no change to `max_count`/downstream array sizing math) and matches CONCERNS.md's own stated fix approach; but this must be an explicit decision recorded in the plan, not left implicit, since it changes what "no degenerate synthetic samples" is interpreted to mean when the plan is later checked against SMOTE-03.
   - **RESOLVED (superseded 2026-07-27, second verification pass, explicit user decision):** Option (b) adopted — NOT option (a) as originally recommended above. The second-pass checker flagged that a logged-but-continued duplicate still literally violates SMOTE-03's "no degenerate synthetic samples" wording (a duplicate is degenerate regardless of whether it is logged, since it is still a synthetic row generated by interpolating a point against itself). The user explicitly chose to skip synthesis entirely for `n_class <= 1`: `log_warn(...)` still fires (unchanged trigger condition), but zero synthetic samples are generated for that class — `n_synthetic` is forced to `0` before the synthesis loop runs, so `find_knn()`/the interpolation math is never invoked at all for that class. The function's `*n_out` is corrected from the pre-allocated `max_count * num_classes` upper bound to the actual `out_idx` row count, so downstream consumers (`mlp_train()` via `os_n_m`/`os_n_e`) never read uninitialized memory for the skipped rows. This edge case is expected to be rare given this dataset's class sizes, so the practical impact (a slightly smaller, class-weight-compensated training set for that specific class/fold/vowel/network combination) is minimal — this tradeoff was explicitly weighed and accepted by the user. See `01-01-PLAN.md` Task 2, Block A.
...
- Pitfalls: HIGH — all four pitfalls tied to specific, verified line numbers or verified algorithm-primary-source details; the one genuinely open design question (the `n_class <= 1` fallback semantics) was flagged explicitly as an Open Question, not asserted as settled, and has since been re-resolved by explicit user decision (skip, not duplicate) after the second verification pass
